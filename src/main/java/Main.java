	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (connection != null) {
				...
				else if (pathPieces[1].equals("search") && pathPieces.length == 7)
					search.doSearch(request, response, connection, pathPieces[2], pathPieces[3], pathPieces[4], pathPieces[5], pathPieces[6]);
				else
					response.setStatus(Constants.NOT_FOUND);
